...
async function loadFarm(id: string) {
  ...
  const [farms, listings, reviews] = await Promise.all([
    sql`SELECT farm.id, farm.name, farm.description, farm.phone, farm.email, farm.address_text, farm.city, farm.state,
...
function ProductCard({ item, showFarm=false }: { item: Row; showFarm?: boolean }) {
  return <article className="store-product-card"><img src={item.image_url?listingImageUrl(String(item.id),item.image_url):DEFAULT_LISTING_IMAGE} alt={String(item.title)}/><div><small>{String(item.category)}</small><h3>{String(item.title)}</h3>{showFarm&&<a href={`/farms/${item.farm_id}`}>{String(item.farm_name)} <ChevronRight size={14}/></a>}<p><strong>{money(item.unit_price_kobo)}</strong> / {String(item.unit)}</p></div></article>;
}
...
export default async function FarmStorePage({ params }: Props) {
  ...
  return <FarmStoreTheme>
    ...
    <main>
    <section className="store-hero"><img src={hero} alt=""/><span/><div><p>VERIFIED LOCAL FARM</p><h1>{String(farm.name)}</h1><div className="store-rating"><Stars rating={rating}/><strong>{rating.toFixed(1)}</strong><em>{Number(farm.review_count)} {Number(farm.review_count)===1?"review":"reviews"}</em><BadgeCheck size={20}/></div></div></section>

...
export async function POST(request: NextRequest) {
  ...
  const [order] = await sql`
    SELECT orders.id, orders.order_number, orders.total_kobo, orders.status, users.email,
      payment.id AS payment_id, payment.provider_reference, payment.authorization_url, payment.status AS payment_status
    FROM orders JOIN users ON users.id = orders.customer_id
    LEFT JOIN LATERAL (SELECT id, provider_reference, authorization_url, status FROM payments WHERE order_id = orders.id AND provider = 'paystack' ORDER BY created_at DESC LIMIT 1) payment ON true
  ...
  if (order.authorization_url && order.payment_status !== "failed") return NextResponse.json({ authorizationUrl: order.authorization_url, reference: order.provider_reference, orderNumber: order.order_number });

  const reference = order.payment_status === "failed" || !order.provider_reference
    ? `HNU-${String(order.order_number).replace(/[^a-z0-9]/gi, "")}-${randomUUID().slice(0, 8)}`
    : order.provider_reference;
  if (!order.payment_id || order.payment_status === "failed") {
  ...
  try {
    const transaction = await initializePaystackTransaction({
      ...
      callbackUrl: `${siteUrl}/api/payments/paystack/callback`, orderId: String(order.id), orderNumber: String(order.order_number),
    });
    await sql`UPDATE payments SET authorization_url = ${transaction.authorization_url}, provider_response = ${JSON.stringify({ accessCode: transaction.access_code })}::jsonb, updated_at = now() WHERE order_id = ${order.id} AND provider = 'paystack' AND provider_reference = ${reference}`;

...
export async function GET() {
  ...
  if (!settings?.is_enabled || !settings.bank_name || !settings.account_name || !settings.account_number) {
    return NextResponse.json({ settings: null, storeCreditKobo: Number(credit?.balance_kobo || 0), manualPaymentAvailable: false, paystackAvailable: paystackEnabled() });
  }
  return NextResponse.json({ settings, storeCreditKobo: Number(credit?.balance_kobo || 0), manualPaymentAvailable: true, paystackAvailable: paystackEnabled() });
}

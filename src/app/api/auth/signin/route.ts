...
export async function POST(request: Request) {
  const body = await request.json().catch(() => null) as { identifier?: string; password?: string; remember?: boolean } | null;
  ...
  const session = await createSession(String(user.id), { credentialVersion: isSuperAdminAccount(user) ? superAdminCredentialVersion() : null, remember: body.remember !== false });

...
type AdminEntityType = "users" | "farms" | "produce" | "pickup_centres" | "orders" | "refunds" | "payouts" | "reviews" | "activity";
...
function adminEntityFilterValue(section: AdminEntityType, entity: AdminEntity) {
  ...
  if (["farms", "produce", "pickup_centres", "orders", "refunds", "payouts"].includes(section)) return String(entity.status || entity.verification_status || "unknown");
...
function AdminPage({ readOnly, supportAccess, onImpersonated }: { readOnly: boolean; supportAccess: boolean; onImpersonated: (user: CurrentUser) => void }) {
  ...
  async function addEntity(event: FormEvent<HTMLFormElement>) {
    ...
    if (["farms", "pickup_centres"].includes(section) && (!values.latitude || !values.longitude)) {
      try {
        ...
        setError(`Allow location access while at the ${section === "farms" ? "farm" : "pickup centre"} so its coordinates can be recorded.`); setBusy(false); return;
      }
    }
  ...
  return <main className="admin-page">
    ...
    <div className="admin-workspace">
      <label className="admin-mobile-section"><span className="sr-only">Administration section</span><select aria-label="Administration section" value={section} onChange={(event) => { const next = event.target.value as typeof section; setSection(next); setSelected(null); setError(""); setBusy(next !== "overview"); }}>{(["overview", "users", "farms", "produce", "pickup_centres", "orders", "refunds", "payouts", "reviews", "activity"] as const).filter((item) => !(supportAccess && item === "activity")).map((item) => <option key={item} value={item}>{item === "produce" ? "Produce listings" : item === "pickup_centres" ? "Pickup centres" : item[0].toUpperCase() + item.slice(1)}{item === "refunds" && metrics.open_refunds > 0 ? ` (${metrics.open_refunds})` : item === "payouts" && metrics.open_payouts > 0 ? ` (${metrics.open_payouts})` : ""}</option>)}</select><ChevronDown size={16}/></label>
      <nav className="admin-tabs" aria-label="Administration sections"><small>Workspace</small>{(["overview", "users", "farms", "produce", "pickup_centres", "orders", "refunds", "payouts", "reviews", "activity"] as const).filter((item) => !(supportAccess && item === "activity")).map((item) => <button key={item} className={section === item ? "active" : ""} onClick={() => { setSection(item); setSelected(null); setError(""); setBusy(item !== "overview"); }}>{item === "overview" ? <House size={15}/> : item === "users" ? <UserRound size={15}/> : item === "farms" ? <Store size={15}/> : item === "produce" ? <Leaf size={15}/> : item === "pickup_centres" ? <MapPin size={15}/> : item === "orders" ? <PackageCheck size={15}/> : item === "refunds" ? <RotateCcw size={15}/> : item === "payouts" ? <CreditCard size={15}/> : item === "reviews" ? <Star size={15}/> : <Clock3 size={15}/>}<span>{item === "pickup_centres" ? "Pickup centres" : item}</span>{item === "refunds" && metrics.open_refunds > 0 && <b className="admin-tab-count">{metrics.open_refunds}</b>}{item === "payouts" && metrics.open_payouts > 0 && <b className="admin-tab-count">{metrics.open_payouts}</b>}</button>)}</nav>
    ...
    {section === "overview" ? <>
      ...
      <div className="entity-toolbar"><div><h2>{section === "produce" ? "Produce listings" : section === "pickup_centres" ? "Pickup centres" : section[0].toUpperCase() + section.slice(1)}</h2><p>{entities.length} database records</p></div>{!readOnly && ["users","farms","produce","pickup_centres"].includes(section) && <button onClick={() => { setError(""); if (section === "pickup_centres") setPickupCentreModal("add"); else setAddOpen(true); }}><Plus size={16}/> Add {section === "produce" ? "produce" : section === "pickup_centres" ? "pickup centre" : section.slice(0, -1)}</button>}</div>
    ...
    {selected && section !== "overview" && <div className="admin-drawer-overlay" onMouseDown={() => setSelected(null)}>
      <aside className="admin-detail" onMouseDown={(event) => event.stopPropagation()}>
        ...
        {!readOnly && section !== "activity" && <footer className="admin-detail-actions">
          ...
          {section !== "payouts" && !(section === "orders" && selected.status === "pending_payment") && <button className="edit-entity" onClick={() => { setError(""); if (section === "pickup_centres") setPickupCentreModal("edit"); else setEditOpen(true); }} disabled={busy}>{["orders","refunds","reviews"].includes(section) ? "Manage record" : "Edit details"}</button>}
          ...
          {["users","farms","produce","pickup_centres"].includes(section) && <button className="remove-entity" onClick={removeEntity} disabled={busy}><Trash2 size={16}/> {section === "pickup_centres" ? "Deactivate centre" : <>Remove {section === "produce" ? "listing" : section.slice(0, -1)}</>}</button>}
        </footer>}
      </aside>
    </div>}
...
function AdminEntityRow({ section, entity, onOpen, onReviewRefund }: { section: AdminEntityType; entity: AdminEntity; onOpen: () => void; onReviewRefund: (refundId: string) => void }) {
  ...
  if (section === "pickup_centres") return <button onClick={onOpen}><span className="entity-icon"><MapPin size={17}/></span><span><strong>{String(entity.name)}</strong><small>{String(entity.address_text)} · {String(entity.city)}, {String(entity.state)}</small><AdminEntityDate label="Created" value={entity.created_at}/></span><b className={`status-badge ${entity.is_active ? "verified" : "suspended"}`}>{entity.is_active ? "Active" : "Inactive"}</b><i>{String(entity.order_count || 0)} orders</i></button>;

...
function ModalBrand() {
  return <div className="modal-brand" aria-hidden="true"><img src="/brand/harvestnearu-opaque-seal-se2-lockup.png" alt="" loading="lazy" decoding="async"/></div>;
}
...
function RequiredLocationSetup({ onSaved }: { onSaved: (user: CurrentUser) => void }) {
  ...
  const pinned = Boolean(latitude && longitude);
  function captureLocation() {
    if (!navigator.geolocation) { setError("This browser cannot share your location. Use Enter coordinates manually instead."); setManualCoordinates(true); return; }
    setLocating(true); setError("");
    navigator.geolocation.getCurrentPosition((position) => { setLatitude(String(position.coords.latitude)); setLongitude(String(position.coords.longitude)); setLocating(false); }, () => { setError("Location access was not granted. Allow it, or use Enter coordinates manually."); setManualCoordinates(true); setLocating(false); }, { enableHighAccuracy: true, timeout: 12000 });
  }
  async function save(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError("");
    if (!pinned) { setError("Pin this address with Use my current location so nearby farms and delivery can be calculated."); setManualCoordinates(true); return; }
    setSaving(true);
  ...
  return <div className="modal-overlay required-location-overlay"><div className="signup-modal required-location-modal"><ModalBrand/><p className="auth-kicker">FINAL ACCOUNT SETUP</p><h2>Save your location</h2><p className="signup-intro">Your location is required to rank nearby harvests, send relevant restock announcements, and calculate delivery.</p><form className="signup-form" onSubmit={save}><label>Phone number<input name="recipientPhone" type="tel" required autoComplete="tel"/></label><label>Street address or area<input name="line1" required autoComplete="street-address"/></label><div className="form-row"><label>City<input name="city" required autoComplete="address-level2"/></label><label>State<input name="state" required autoComplete="address-level1" defaultValue="FCT"/></label></div><div className="location-pin"><button type="button" className={`capture-location-button ${pinned ? "pinned" : ""}`} onClick={captureLocation} disabled={locating}>{pinned && !locating ? <Check size={16}/> : <LocateFixed size={16}/>}{locating ? "Finding your location..." : pinned ? "Location pinned · tap to update" : "Use my current location"}</button>{!pinned && <p className="location-pin-hint">This pins the address on the map so we can rank nearby farms and price delivery.</p>}{!manualCoordinates ? <button type="button" className="manual-coordinates-toggle" onClick={()=>setManualCoordinates(true)}>Enter coordinates manually</button> : <div className="form-row"><label>Latitude<input type="number" min="-90" max="90" step="any" value={latitude} onChange={(event)=>setLatitude(event.target.value)}/></label><label>Longitude<input type="number" min="-180" max="180" step="any" value={longitude} onChange={(event)=>setLongitude(event.target.value)}/></label></div>}</div>{error&&<p className="auth-error" role="alert">{error}</p>}<button className="create-account" disabled={saving}>{saving ? "Saving location..." : "Save and continue"}</button></form></div></div>;
}
...
function getThemeSnapshot(): Theme {
  // A stored choice always wins. Without one, follow the device: a reader whose phone is in dark
  // mode should not be handed a white page and have to go looking for the switch.
  const stored = localStorage.getItem("harvest-near-theme");
...
function FarmCoordinateFields({ defaultLatitude = "", defaultLongitude = "", subject = "farm" }: { defaultLatitude?: string | number; defaultLongitude?: string | number; subject?: "farm" | "home" }) {
  const [latitude, setLatitude] = useState(String(defaultLatitude));
  const [longitude, setLongitude] = useState(String(defaultLongitude));
  const [locating, setLocating] = useState(false);
  const [manualCoordinates, setManualCoordinates] = useState(false);
  const isFarm = subject === "farm";
  const pinned = Boolean(latitude && longitude);
  function captureLocation() {
    if (!navigator.geolocation) { setManualCoordinates(true); return; }
    setLocating(true);
    navigator.geolocation.getCurrentPosition((position) => {
      setLatitude(position.coords.latitude.toFixed(6));
      setLongitude(position.coords.longitude.toFixed(6));
      setLocating(false);
    }, () => { setManualCoordinates(true); setLocating(false); }, { enableHighAccuracy: true, timeout: 12000 });
  }
  const showCoordinates = isFarm || manualCoordinates;
  return <div className="farm-coordinate-fields"><button type="button" className={pinned && !isFarm ? "pinned" : ""} onClick={captureLocation}>{pinned && !isFarm && !locating ? <Check size={15}/> : <LocateFixed size={15}/>}{locating ? "Finding the location..." : isFarm ? "Use farm's current location" : pinned ? "Location pinned · tap to update" : "Use my current location"}</button>{showCoordinates && <div className="form-row"><label>Latitude<input name="latitude" type="number" min="-90" max="90" step="any" value={latitude} onChange={(event) => setLatitude(event.target.value)} placeholder="9.076500" required={isFarm}/></label><label>Longitude<input name="longitude" type="number" min="-180" max="180" step="any" value={longitude} onChange={(event) => setLongitude(event.target.value)} placeholder="7.465100" required={isFarm}/></label></div>}{!showCoordinates && <input type="hidden" name="latitude" value={latitude}/>}{!showCoordinates && <input type="hidden" name="longitude" value={longitude}/>}<small>{isFarm ? "Capture this while physically at the farm, or enter its map coordinates." : pinned ? "This pins your address on the map so nearby farms rank correctly and delivery can be priced." : "Pinning your address lets nearby farms rank correctly and delivery be priced."}</small>{!isFarm && !manualCoordinates && <button type="button" className="manual-coordinates-toggle" onClick={() => setManualCoordinates(true)}>Enter coordinates manually</button>}</div>;
}
  ...
  return (
    <div className="app-shell" data-theme={theme}>
      ...
      <header className="topbar">
        <button className="brand brand-image" onClick={() => navigate(isAdmin ? "admin" : "landing")} aria-label={isAdmin ? "HarvestNearU administration" : "HarvestNearU home"}><img className="brand-lockup" src="/brand/harvestnearu-opaque-seal-se2-lockup.png" alt="HarvestNearU"  decoding="async"/></button>
        ...
        <div className="header-actions">
          ...
          <div className="account-menu-wrap" ref={accountMenuRef}>
            <button className={`account-menu-trigger ${accountMenuOpen ? "active" : ""}`} onClick={() => setAccountMenuOpen((open) => !open)} aria-expanded={accountMenuOpen} aria-haspopup="menu" disabled={sessionLoading}>
              <span className={`account-avatar ${currentUser?.avatarUrl ? "has-photo" : ""}`}>{currentUser?.avatarUrl ? <img src={currentUser.avatarUrl} alt="" loading="lazy" decoding="async"/> : <UserRound size={17} />}</span><ChevronDown size={15} /><span className="sr-only">Account menu</span>
            </button>
            {accountMenuOpen && <>
              <div className="account-menu" role="menu">
                <div className="account-menu-heading"><span className={`account-avatar ${currentUser?.avatarUrl ? "has-photo" : ""}`}>{currentUser?.avatarUrl ? <img src={currentUser.avatarUrl} alt="" loading="lazy" decoding="async"/> : <UserRound size={17} />}</span><div><strong>{currentUser ? `${currentUser.firstName} ${currentUser.lastName}` : sessionLoading ? "Checking your account" : "Welcome to HarvestNearU"}</strong><small>{currentUser ? roleLabel(currentUser.role) : "Manage your account and preferences"}</small></div></div>
        ...
        <main>
          ...
          <section className="catalog">
            {query.trim() && (effectiveIntentLoading || matchedIntentTerms.length > 0 || (localSearchMatchCount === 0 && intentResolvedQuery === searchInput)) && <div className="search-intent-status" role="status" aria-live="polite">
              <img src="/brand/amara-avatar.png" alt="Amara, HarvestNearU assistant" style={{width:34,height:34,objectFit:"cover",borderRadius:9,flex:"none"}} loading="lazy" decoding="async"/><span><strong>{effectiveIntentLoading ? "Amara is looking for broader matches..." : effectiveIntentEnhanced ? `Amara suggests: ${effectiveIntentExplanation}` : effectiveIntentExplanation}</strong>{matchedIntentTerms.length > 0 && <small>Matching available produce: {matchedIntentTerms.join(", ")}</small>}</span>
            </div>}
      ...
      {signupOpen && <div className="modal-overlay" onMouseDown={() => setSignupOpen(false)}><div className="signup-modal" onMouseDown={(event) => event.stopPropagation()}>
        ...
        {!signupComplete ? <>
          <div className="signup-heading"><span className="auth-logo-lockup"><img className="auth-approved-lockup" src="/brand/harvestnearu-opaque-seal-se2-lockup.png" alt="HarvestNearU"  loading="lazy" decoding="async"/></span><div><p>JOIN HARVESTNEARU</p><h2>Create your account</h2></div></div>
          ...
          <form className="signup-form" onSubmit={submitSignup}>
            ...
            <label className="terms"><input required type="checkbox" /> <span>I agree to the <a href="/terms" target="_blank" rel="noreferrer">Terms of Service</a> and <a href="/privacy" target="_blank" rel="noreferrer">Privacy Policy</a>.</span></label>
      ...
      {signinOpen && <div className="modal-overlay" onMouseDown={closeSignIn}><div className="signin-modal" onMouseDown={(event) => event.stopPropagation()}>
        ...
        {!signinComplete && recoveryStage === "signin" ? <>
          <div className="auth-logo-lockup signin-brand"><img className="auth-approved-lockup" src="/brand/harvestnearu-opaque-seal-se2-lockup.png" alt="HarvestNearU"  loading="lazy" decoding="async"/></div>
...
function LandingPage({ stats, signedOut, onShop, onFarmer, onSignup }: { stats: MarketplaceStats | null; signedOut: boolean; onShop: () => void; onFarmer: () => void; onSignup: () => void }) {
  return <main className="landing-page">
    <section className="landing-hero">
      <img src="/produce/vine-ripe-tomatoes.webp" alt="Fresh tomatoes harvested by a local farmer"  loading="lazy" decoding="async"/>
      ...
      <aside className="hero-harvest-note"><span>ACTIVE HARVESTS</span><strong>{stats ? `${stats.listings} fresh listings` : "Loading harvests"}</strong><p>{stats ? `from ${stats.farms} verified farms near Abuja` : "Checking nearby farms"}</p><div><img src="/produce/fresh-sweet-corn.webp" alt="" loading="lazy" decoding="async"/><img src="/produce/garden-fresh-spinach.webp" alt="" loading="lazy" decoding="async"/><img src="/produce/sweet-ripe-plantain.webp" alt="" loading="lazy" decoding="async"/></div></aside>
    </section>
    ...
    <section className="audience-band consumer-band">
      <div className="audience-image"><img src="/produce/creamy-avocados.webp" alt="Fresh avocados from a local farm" loading="lazy" decoding="async"/><span title="2.4 km straight-line distance"><strong>{proximityLabel(2.4)}</strong> from your location</span></div>
    ...
    <section className="audience-band farmer-band">
      ...
      <div className="audience-image"><img src="/produce/oyo-white-yam.webp" alt="Fresh yam ready for market" loading="lazy" decoding="async"/><span><strong>72%</strong> of this harvest sold</span></div>
    </section>

    <section className="landing-cta"><img className="outlined-brand-mark" src="/brand/harvestnearu-mark-outline.png" alt="HarvestNearU mark" loading="lazy" decoding="async"/><div><p>YOUR LOCAL HARVEST IS WAITING</p><h2>Find something fresh nearby.</h2><span>Start with today&apos;s produce and choose the journey that works for you.</span></div><button onClick={onShop}>Browse the market <ArrowRight size={17}/></button></section>
  </main>;
}
  ...
  return <main className="support-page">
    ...
    {page === "returns" && <section className="support-content">
      ...
      <div className="policy-grid">
        ...
        <article className="policy-card"><span><AtSign size={20}/></span><h3>Credit or bank refund</h3><p>Account credit is free and is applied to your next order. A refund to your bank instead carries a ₦500 cancellation fee, deducted from the amount returned.</p></article>
      </div>
  ...
  return <footer className="site-footer">
    <div className="footer-main">
      <div className="footer-brand">
        <button className="footer-logo" onClick={() => onNavigate("landing")} aria-label="HarvestNearU home"><img src="/brand/harvestnearu-opaque-seal-se2-lockup.png" alt="HarvestNearU"  loading="lazy" decoding="async"/></button>
    ...
    <div className="footer-bottom"><span>© 2026 HarvestNearU Nigeria</span><div><a href="/privacy">Privacy</a><a href="/terms">Terms</a><a href="/account-deletion">Delete account</a></div><span className="footer-local"><MapPin size={12}/> Fresh Local Produce, Found Here</span></div>
  </footer>;
}
...
function HarvestSpinner({ compact = false, label }: { compact?: boolean; label: string }) {
  return <div className={`harvest-spinner ${compact ? "compact" : ""}`} role="status" aria-label={label}>
    <span className="spinner-orbit"><i/><b/><span><img src="/brand/harvestnearu-approved-mark.png" alt="" loading="lazy" decoding="async"/></span></span>
  </div>;
}
...
function DatabaseProfilePage({ role, onShop, onFarmer, onUpgraded }: { role: "consumer" | "farmer"; onShop: () => void; onFarmer: () => void; onUpgraded: (user: CurrentUser) => void }) {
  ...
  async function saveProfile(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const values = Object.fromEntries(new FormData(event.currentTarget).entries());
      const response = await fetch("/api/profile", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...values, marketingConsent: role === "consumer" ? values.marketingConsent === "true" : undefined, offersPickup: values.offersPickup === "true", offersDelivery: values.offersDelivery === "true" }) });
  ...
  return <main className="profile-page" aria-busy={Boolean(switchingFarmId)}>
    ...
    <section className="profile-identity"><div className="identity-cover consumer-cover"><img src={role === "farmer" ? "/produce/fresh-sweet-corn.webp" : "/produce/garden-fresh-spinach.webp"} alt="Fresh produce" loading="lazy" decoding="async"/><div/></div><div className="identity-row"><span className={`profile-avatar ${data.user.avatar_url ? "has-photo" : ""}`}>{data.user.avatar_url ? <img src={data.user.avatar_url} alt={name} loading="lazy" decoding="async"/> : initials}</span><div><span className="verified-label"><Check size={11}/> {role === "farmer" ? data.farm?.verification_status || "pending" : data.user.email_verified_at ? "Verified customer" : "Customer account"}</span><h2>{role === "farmer" ? data.farm?.name || name : name}</h2><p><MapPin size={13}/> {role === "farmer" ? `${data.farm?.city || ""}, ${data.farm?.state || ""}` : data.addresses[0] ? `${data.addresses[0].city}, ${data.addresses[0].state}` : "Add your delivery address"} · Member since {new Date(data.user.created_at).getFullYear()}</p></div></div></section>
...
type CustomerOrder = {
  ...
  farms: Array<{ id: string; name: string; rating: number | null; comment: string | null; unread_messages?: number }>;
...
function CustomerOrderCard({ order, active, expanded, receiptBusy, onToggle, onUpload, onPaystack, onCancel, onConfirm, onRating, onChat }: { order: CustomerOrder; active: boolean; expanded: boolean; receiptBusy: boolean; onToggle: () => void; onUpload: (file: File | undefined) => void; onPaystack: () => void; onCancel: () => void; onConfirm: (item: CustomerOrder["items"][number]) => void; onRating: (farm: CustomerOrder["farms"][number]) => void; onChat: (farm: CustomerOrder["farms"][number]) => void }) {
  ...
  return <article className="database-order combined-order">
    ...
    {order.fulfilment_method === "farmer_delivery" && <div className="order-chat-actions"><strong>Arrange delivery</strong>{order.farms.map((farm) => <button key={farm.id} onClick={() => onChat(farm)}><MessageCircle size={14}/> {farm.name}{Number(farm.unread_messages) > 0 && <b className="chat-unread" aria-label={`${farm.unread_messages} unread messages`}>{Number(farm.unread_messages) > 9 ? "9+" : farm.unread_messages}</b>}</button>)}</div>}
    ...
    {active && <div className={`order-detail-collapse active-product-tracking ${expanded ? "open" : ""}`} aria-hidden={!expanded}><div><section className="live-tracking">
      ...
      <div className="item-tracking-list">{order.items.map((item) => {
        ...
        return <article className={received ? "received" : ""} key={item.id}>
          <div className="item-tracking-heading"><button className="item-tracking-main" onClick={() => toggleItem(item.id)} aria-expanded={itemOpen}>{item.image ? <img src={item.image} alt="" loading="lazy" decoding="async"/> : <span><Leaf size={17}/></span>}<p><strong>{item.name}</strong><small>{item.farm} · {item.quantity} {item.unit}</small></p><div className="item-tracking-meta"><strong>{money(Number(item.unit_price_kobo) * Number(item.quantity) / 100)}</strong><b className={`status-badge ${item.status}`}>{statusLabel(received ? "received" : item.status)}</b></div></button><button className="item-expand-button" onClick={() => toggleItem(item.id)} aria-expanded={itemOpen} aria-label={`${itemOpen ? "Hide" : "Show"} tracking progress for ${item.name}`}><ChevronDown className={itemOpen ? "open" : ""} size={18}/></button></div>
    ...
    <div className={`order-detail-collapse ${expanded ? "open" : ""}`} aria-hidden={!expanded}><div><div className="database-order-detail">{!active && <div className="database-order-items">{order.items.map((item) => <div key={item.id}>{item.image ? <img src={item.image} alt="" loading="lazy" decoding="async"/> : <span><Leaf size={18}/></span>}<p><strong>{item.name}</strong><small>{item.quantity} {item.unit} · {item.farm}</small></p><b>{money(Number(item.unit_price_kobo) * Number(item.quantity) / 100)}</b></div>)}</div>}{!active && <div className="order-farm-ratings">{order.farms.map((farm) => <div key={farm.id}><span><strong>{farm.name}</strong><small>{farm.rating ? `Your rating: ${farm.rating}/5` : "Share your experience with this farm"}</small></span><button onClick={() => onRating(farm)}><Star size={14} fill={farm.rating ? "currentColor" : "none"}/> {farm.rating ? "Edit rating" : "Rate farm"}</button></div>)}</div>}<div className="database-order-meta"><span><small>FULFILMENT</small><strong>{order.fulfilment_method.replaceAll("_", " ")}</strong></span><span><small>DELIVERY</small><strong>{money(Number(order.delivery_fee_kobo) / 100)}</strong></span><span><small>TOTAL</small><strong>{money(Number(order.total_kobo) / 100)}</strong></span></div></div></div></div>
...
const DEFAULT_FEE_POLICY = { rate: 0.1, percent: 10, label: "10%" };

/**
 * The price field a farmer sells at, with the fee that will be deducted stated beside it and the
 * resulting payout worked out as they type, so nobody discovers the deduction at payout time.
 */
function PriceWithFeeNotice({ fee, defaultValue }: { fee: { rate: number; label: string }; defaultValue?: number }) {
  ...
  return <label>Price (NGN)
    <input name="price" type="number" min="1" defaultValue={defaultValue} onChange={(event) => setPrice(event.target.value)} required/>
    <small className="fee-note">
      A {fee.label} administration and processing fee is deducted from each sale.
      {receives === null ? " The rate may be reviewed in future." : <> You receive <strong>{money(receives)}</strong> per unit sold. The rate may be reviewed in future.</>}
    </small>
  </label>;
}
...
type FarmerWorkspaceData = {
  user: CurrentUser;
  farm: { id: string; name: string; verification_status: string; average_rating: number; review_count: number; delivery_radius_km: number; offers_delivery: boolean };
  ...
  fees?: { rate: number; percent: number; label: string };
...
function FarmerWorkspace({ onShop }: { onShop: () => void }) {
  ...
  const [resolvedAccount, setResolvedAccount] = useState<{ accountName: string; accountLast4: string } | null>(null);
  const [resolving, setResolving] = useState(false);
  const [accountConfirmed, setAccountConfirmed] = useState(false);
  const [payoutPasswordRequired, setPayoutPasswordRequired] = useState(false);
  ...
  async function openPayoutAccount() {
    ...
    try { const response=await fetch(`/api/farmer/payout-account?farmId=${data.farm.id}`);const result=await readJsonResponse<{account:null|{bank_name?:string;account_name?:string;account_last4?:string};banks?:Array<{name:string;code:string}>;passwordRequired?:boolean;error?:string}>(response);if(!response.ok)throw new Error(result.error||"Could not load payout account");setPayoutAccount(result.account);setPayoutBanks(result.banks||[]);setPayoutPasswordRequired(Boolean(result.passwordRequired));openPayoutAccountDialog();} catch(reason){setError((reason as Error).message);} finally{setBusy(false);}
  }
  function closePayoutAccountDialog(){setPayoutAccountOpen(false);setResolvedAccount(null);setAccountConfirmed(false);setPayoutPasswordRequired(false);}
  function openPayoutAccountDialog(){setResolvedAccount(null);setAccountConfirmed(false);setPayoutAccountOpen(true);}

  /** Asks the bank who owns the account so the farmer can check the name before anything is saved. */
  async function checkPayoutAccount(bankCode:string,accountNumber:string){
    setResolvedAccount(null);setAccountConfirmed(false);setError("");
    if(!data||!bankCode||!/^\d{10}$/.test(accountNumber))return;
    setResolving(true);
    try{
      const response=await fetch("/api/farmer/payout-account/resolve",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({farmId:data.farm.id,bankCode,accountNumber})});
      const result=await readJsonResponse<{accountName?:string;accountLast4?:string;error?:string}>(response);
      if(!response.ok||!result.accountName)throw new Error(result.error||"Could not check this account number");
      setResolvedAccount({accountName:result.accountName,accountLast4:result.accountLast4||accountNumber.slice(-4)});
    }catch(reason){setError((reason as Error).message);}
    finally{setResolving(false);}
  }

  async function savePayoutAccount(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!data)return;setBusy(true);setError("");try{const form=new FormData(event.currentTarget);const bankCode=String(form.get("bankCode")||"");const bank=payoutBanks.find((item)=>item.code===bankCode);const response=await fetch("/api/farmer/payout-account",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({farmId:data.farm.id,bankCode,bankName:bank?.name,accountNumber:String(form.get("accountNumber")||""),confirmedAccountName:resolvedAccount?.accountName,password:payoutPasswordRequired?String(form.get("password")||""):undefined})});const result=await readJsonResponse<{account?:typeof payoutAccount;error?:string}>(response);if(!response.ok||!result.account)throw new Error(result.error||"Could not save payout account");setPayoutAccount(result.account);closePayoutAccountDialog();}catch(reason){setError((reason as Error).message);}finally{setBusy(false);}}

  async function saveDelivery(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!data) return;
    setBusy(true); setError(""); setDeliverySaved("");
    try {
      const form = new FormData(event.currentTarget);
      const response = await fetch("/api/farmer/dashboard", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "delivery", farmId: data.farm.id, deliveryRadius: String(form.get("deliveryRadius") ?? ""), offersDelivery: form.get("offersDelivery") === "true" }) });
      const result = await readJsonResponse<{ error?: string }>(response);
      if (!response.ok) throw new Error(result.error || "Could not save delivery settings");
  ...
  return <main className="farmer-page">
    ...
    <section className="farmer-delivery-settings"><header><div><small>DOORSTEP DELIVERY</small><h2>How far you deliver</h2><p>Customers pay ₦250 per kilometre to the farm, so your radius also sets the most a delivery from here can cost them.</p></div><span className={data.farm.offers_delivery && Number(data.farm.delivery_radius_km) > 0 ? "on" : "off"}>{data.farm.offers_delivery && Number(data.farm.delivery_radius_km) > 0 ? <><Truck size={14}/> Delivering up to {Number(data.farm.delivery_radius_km)} km</> : <><Store size={14}/> Pickup only</>}</span></header><form onSubmit={saveDelivery}><label>Delivery radius (km)<input name="deliveryRadius" type="number" min="0" max={data.deliveryRadiusMaxKm ?? 9999.99} step="0.01" defaultValue={Number(data.farm.delivery_radius_km)} disabled={busy || Boolean(data.user.impersonating)}/><small>A customer further away than this is offered farm pickup instead. At {Number(data.farm.delivery_radius_km) || 0} km the longest delivery costs {money((Number(data.farm.delivery_radius_km) || 1) * 250)}.</small></label><label className="admin-check"><input name="offersDelivery" type="checkbox" value="true" defaultChecked={data.farm.offers_delivery} disabled={busy || Boolean(data.user.impersonating)}/> Offer doorstep delivery</label><button className="admin-submit" disabled={busy || Boolean(data.user.impersonating)}>{busy ? "Saving..." : "Save delivery settings"}</button></form>{deliverySaved && <p className="farmer-delivery-saved" role="status">{deliverySaved}</p>}</section>
    ...
    {payoutAccountOpen&&<div className="modal-overlay" onMouseDown={()=>closePayoutAccountDialog()}><div className="admin-add-modal payout-account-modal" onMouseDown={(event)=>event.stopPropagation()}><button className="close-modal" onClick={()=>closePayoutAccountDialog()}><X size={19}/></button><p className="auth-kicker">PAYOUT ACCOUNT</p><h2>{data.farm.name}</h2><p>Paystack verifies the account and HarvestNearU stores only the recipient token and last four digits.</p>{payoutAccount?.account_last4&&<div className="payout-account-current"><Check size={17}/><span><strong>{payoutAccount.account_name}</strong><small>{payoutAccount.bank_name} · ending {payoutAccount.account_last4}</small></span></div>}<form onSubmit={savePayoutAccount} onChange={(event)=>{if((event.target as unknown as {name?:string}).name==="password")return;const form=event.currentTarget;const bankCode=String(new FormData(form).get("bankCode")||"");const accountNumber=String(new FormData(form).get("accountNumber")||"");void checkPayoutAccount(bankCode,accountNumber);}}><label>Bank<select name="bankCode" required defaultValue=""><option value="" disabled>Select bank</option>{payoutBanks.map((bank)=><option key={bank.code} value={bank.code}>{bank.name}</option>)}</select></label><label>Account number<input name="accountNumber" inputMode="numeric" pattern="[0-9]{10}" minLength={10} maxLength={10} required placeholder="10-digit NUBAN account"/></label>{resolving&&<p className="payout-resolving"><LoaderCircle size={15}/> Checking the account with the bank...</p>}{resolvedAccount&&<div className="payout-resolved"><small>THIS ACCOUNT BELONGS TO</small><strong>{resolvedAccount.accountName}</strong><span>Ending {resolvedAccount.accountLast4}. Payouts for this farm will be sent here.</span><label className="payout-confirm"><input type="checkbox" checked={accountConfirmed} onChange={(event)=>setAccountConfirmed(event.target.checked)}/> This is the correct account name</label></div>}{resolvedAccount&&payoutPasswordRequired&&<label>Your password<input name="password" type="password" autoComplete="current-password" required placeholder="Confirm it is you"/><small>Changing where your earnings are sent needs your password, even though you are already signed in.</small></label>}{error&&<p className="admin-error">{error}</p>}<button className="admin-submit" disabled={busy||resolving||!resolvedAccount||!accountConfirmed}>{busy?"Saving account...":resolvedAccount?"Save payout account":"Enter a bank and account number"}</button></form></div></div>}
    {manageListing && <div className="modal-overlay" onMouseDown={() => setManageListing(null)}><div className="admin-add-modal farmer-listing-modal" onMouseDown={(event) => event.stopPropagation()}><button className="close-modal" onClick={() => setManageListing(null)}><X size={19}/></button><p className="auth-kicker">EDIT HARVEST</p><h2>{manageListing.title}</h2><p>Update the listing details shown to customers.</p><form onSubmit={updateInventory}><label>Category<select name="categoryId" defaultValue={manageListing.category_id} required>{data.categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label><label>Produce name<input name="name" defaultValue={manageListing.title} required/></label><div className="form-row"><label>Unit<input name="unit" defaultValue={manageListing.unit} required/></label><PriceWithFeeNotice fee={data.fees || DEFAULT_FEE_POLICY} defaultValue={Number(manageListing.unit_price_kobo) / 100}/></div><div className="form-row"><label>Available quantity<input name="stock" type="number" min={Number(manageListing.quantity_reserved)} defaultValue={Number(manageListing.quantity_available)} required/></label><label>Harvest date<input name="harvestDate" type="date" defaultValue={String(manageListing.harvest_date).slice(0, 10)} required/></label></div><div className="form-row"><label>Available from (optional)<input name="availableFrom" type="datetime-local" defaultValue={lagosDateTimeInput(manageListing.available_from)}/></label><label>Available until (optional)<input name="availableUntil" type="datetime-local" defaultValue={lagosDateTimeInput(manageListing.available_until)}/></label></div>{manageListing.image_url && <div className="listing-image-preview"><img src={manageListing.image_url} alt={`Current ${manageListing.title}`} loading="lazy" decoding="async"/><span>Current picture</span></div>}<label>Change picture<input name="image" type="file" accept="image/png,image/jpeg,image/webp"/><small>Leave empty to keep the current picture. Maximum 4 MB.</small></label><label>Badge<input name="badge" defaultValue={manageListing.badge || ""} placeholder="Picked today"/></label><label>Listing status<select name="status" defaultValue={manageListing.status === "paused" ? "paused" : "active"}><option value="active">Active</option><option value="paused">Paused</option></select></label>{error && <p className="admin-error">{error}</p>}<button className="admin-submit" disabled={busy}>{busy ? "Saving..." : "Save listing"} {!busy && <ArrowRight size={16}/>}</button></form></div></div>}
  </main>;
}

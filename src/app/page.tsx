...
function AdminEntityRow({ section, entity, onOpen }: { section: AdminEntityType; entity: AdminEntity; onOpen: () => void }) {
  ...
  if (section === "orders") return <button onClick={onOpen}><span className="entity-icon"><PackageCheck size={17}/></span><span><strong>Order #{String(entity.order_number)}</strong><small>{String(entity.customer_name)} · {String(entity.item_count)} items</small><AdminEntityDate label="Placed" value={entity.placed_at}/></span><b className={`status-badge ${entity.status}`}>{String(entity.status).replaceAll("_", " ")}</b><i>{money(Number(entity.total_kobo) / 100)}</i></button>;

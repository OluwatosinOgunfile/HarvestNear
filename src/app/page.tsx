...
function AdminPage({ readOnly, supportAccess, onImpersonated }: { readOnly: boolean; supportAccess: boolean; onImpersonated: (user: CurrentUser) => void }) {
  ...
  return <main className="admin-page">
    ...
    <div className="admin-workspace">
      <label className="admin-mobile-section"><span className="sr-only">Administration section</span><select aria-label="Administration section" value={section} onChange={(event) => { const next = event.target.value as typeof section; setSection(next); setSelected(null); setError(""); setBusy(next !== "overview"); }}>{(["overview", "users", "farms", "produce", "orders", "refunds", "reviews", "activity"] as const).filter((item) => !(supportAccess && item === "activity")).map((item) => <option key={item} value={item}>{item === "produce" ? "Produce listings" : item[0].toUpperCase() + item.slice(1)}{item === "refunds" && metrics.open_refunds > 0 ? ` (${metrics.open_refunds})` : ""}</option>)}</select><ChevronDown size={16}/></label>
      <nav className="admin-tabs" aria-label="Administration sections"><small>Workspace</small>{(["overview", "users", "farms", "produce", "orders", "refunds", "reviews", "activity"] as const).filter((item) => !(supportAccess && item === "activity")).map((item) => <button key={item} className={section === item ? "active" : ""} onClick={() => { setSection(item); setSelected(null); setError(""); setBusy(item !== "overview"); }}>{item === "overview" ? <House size={15}/> : item === "users" ? <UserRound size={15}/> : item === "farms" ? <Store size={15}/> : item === "produce" ? <Leaf size={15}/> : item === "orders" ? <PackageCheck size={15}/> : item === "refunds" ? <RotateCcw size={15}/> : item === "reviews" ? <Star size={15}/> : <Clock3 size={15}/>}<span>{item}</span>{item === "refunds" && metrics.open_refunds > 0 && <b className="admin-tab-count">{metrics.open_refunds}</b>}</button>)}</nav>

...
export default function Home() {
  ...
  async function completeOrder() {
    ...
    try {
      if (!paymentReceipt) throw new Error("Select your payment receipt");
      ...
      const result = await response.json() as { orderId?: string; orderNumber?: string; error?: string };
  ...
  return (
    <div className="app-shell" data-theme={theme}>
      ...
      {checkout && <div className="modal-overlay"><div className="payment-modal">
        {!paid ? <><button className="close-modal" onClick={() => setCheckout(false)}><X size={20} /></button><div className="pay-icon"><Leaf size={24} /></div><p className="eyebrow center">MANUAL PAYMENT</p><h2>Submit your transfer receipt</h2><p>Transfer the amount shown, then upload your receipt for administrator confirmation.</p><div className="pay-summary"><span>Total to pay</span><strong>{money(subtotal + deliveryFee)}</strong></div><label>Email address<input value={currentUser?.email || ""} readOnly /></label><label className="payment-receipt-field">Payment receipt<input type="file" accept="image/png,image/jpeg,image/webp,application/pdf" onChange={(event) => setPaymentReceipt(event.target.files?.[0] || null)} disabled={checkoutBusy}/><small>JPG, PNG, WebP, or PDF · Maximum 5 MB</small></label>{checkoutError && <p className="auth-error" role="alert">{checkoutError}</p>}<button className="pay-button" disabled={checkoutBusy || !paymentReceipt} onClick={completeOrder}>{checkoutBusy ? "Submitting receipt..." : "Place order and submit receipt"} {!checkoutBusy && <ArrowRight size={18} />}</button><small>Your order remains pending until an administrator verifies the transfer.</small></> : <div className="success-state"><span><Check size={30} /></span><p className="eyebrow center">RECEIPT SUBMITTED</p><h2>Payment review is pending.</h2><p>Order <strong>#{confirmedOrderNumber}</strong> is reserved. We will notify you after an administrator confirms your payment.</p><button onClick={() => { setCheckout(false); setPaid(false); setPaymentReceipt(null); setCart({}); fetch("/api/cart", { method: "DELETE" }); navigate("orders"); }}>View order status</button></div>}
      </div></div>}
  ...
  return <main className="admin-page">
    ...
    {selected && section !== "overview" && <div className="admin-drawer-overlay" onMouseDown={() => setSelected(null)}>
      <aside className="admin-detail" onMouseDown={(event) => event.stopPropagation()}>
        ...
        {section === "orders" && Boolean(selected.payment_receipt_name) && <section className="admin-payment-review"><span><PackageCheck size={19}/></span><div><strong>Manual payment receipt</strong><small>Submitted {formatEntityValue("submitted_at", selected.payment_receipt_submitted_at)}</small></div><a href={`/api/payments/manual/${selected.id}`} target="_blank" rel="noreferrer">Open receipt</a></section>}
...
function AdminOrderRow({ entity, onOpen }: { entity: AdminEntity; onOpen: () => void }) {
  ...
  return <div className={`admin-order-row ${expanded ? "expanded" : ""}`}>
    <div className="admin-order-summary">
      ...
      <span><strong>Order #{String(entity.order_number)}</strong><small>{String(entity.customer_name)} · {String(entity.item_count)} items</small><span className="entity-farms"><Store size={11}/>{String(entity.farm_names || "Farm not assigned")}</span>{Boolean(entity.receipt_submitted) && <span className="entity-payment-receipt"><Check size={11}/> Receipt ready for review</span>}<AdminEntityDate label="Placed" value={entity.placed_at}/></span>

...
function FarmerWorkspace({ onShop }: { onShop: () => void }) {
  ...
  async function refresh(farmId?: string) {
    const response = await fetch(`/api/farmer/dashboard${farmId ? `?farmId=${encodeURIComponent(farmId)}` : ""}`, { cache: "no-store" });

...
The sweep needs a scheduler. Vercel's Hobby plan allows two cron jobs on daily schedules and both are already used by the notification crons, so `.github/workflows/payout-run.yml` calls the endpoint every ten minutes instead. Add `APP_URL` and `PAYOUT_RUN_SECRET` as repository secrets to enable it. On a paid Vercel plan, add `{ "path": "/api/payouts/run", "schedule": "*/10 * * * *" }` to `vercel.json` and disable the workflow. An administrator can also trigger a run at any time by posting to `/api/payouts/run` while signed in, which needs no secret.
